...
const Profile: React.FC<Props> = ({ user, profile, onSignOut }) => {
  const logsCount = profile?.totalLogs || 0;
  const points = profile?.totalPoints || logsCount * 125;
  const level = Math.floor(logsCount / 5) + 1;
  const progressToNextLevel = logsCount === 0 ? 0 : ((logsCount % 5) / 5) * 100;
  const totalCarbon = profile?.totalCarbon?.toFixed ? profile.totalCarbon.toFixed(1) : '0.0';
  ...
  return (
    <div className="animate-in slide-in-from-right-4 duration-300">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-6 text-center">
        <div className="relative inline-block mb-4">
          <div className="w-24 h-24 bg-gradient-to-br from-green-100 to-green-200 rounded-full flex items-center justify-center text-4xl border-4 border-white shadow-md">
            {profile?.photoURL || user.photoURL ? (
              <img src={profile?.photoURL || user.photoURL || ''} alt="User" className="w-full h-full rounded-full object-cover" />
            ) : (
              '👤'
            )}
          </div>
        ...
        <h2 className="text-xl font-bold text-gray-800">{profile?.displayName || user.displayName || 'Eco Explorer'}</h2>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 p-4 rounded-2xl">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total CO2e</p>
            <p className="text-xl font-black text-green-600">{totalCarbon} kg</p>
          </div>
          <div className="bg-gray-50 p-4 rounded-2xl">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Streak</p>
            <p className="text-xl font-black text-green-600 flex items-center gap-1">{streak} days <span>🔥</span></p>
          </div>
        </div>
